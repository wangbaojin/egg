...
class FriendController extends ApiController
{
    ...
    public function getInviteBuyList()
    {
        ...
        $list = $f_m->field('friend_id as user_id')->where($map)->page($page,$data['page_limit'])->order('id desc')->select();
        ...
        while (list($k,$v)=each($list))
        {
            ...
            $list[$k]['user_pic']       = $user_info['data']['wechart_info']['wx_pic'];
            $list[$k]['user_full_name'] = $user_info['data']['wechart_info']['wx_nick_name'];
            ...
            else
            {
                ...
                $list[$k]['buy_state'] = $m->where($invite_map)->find() ? 2 : 3;
            }
        }
        $data['data'] = $list;
