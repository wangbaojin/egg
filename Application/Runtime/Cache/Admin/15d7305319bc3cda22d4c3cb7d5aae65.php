		        ...
		        <table class="table">
		            ...
		            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr style="border: 1px solid rgb(206,206,206);">
		                    <td  style="padding: 10px">
		                    	<img src="<?php echo ($vo["pic"]); ?>" width="50px" height="50px" >
		                    	<span><?php echo ($vo["activity_info"]); ?></span>
		                    </td>
		                    <td  style="padding: 10px"><?php echo ($vo["newstime_date"]); ?></td>
		                    <td>
		                        <a href="<?php echo U('AdminAdvertisement/edit');?>?id=<?php echo ($vo["id"]); ?>" class="btn btn-info btn-xs" style="margin-right: 10px;">编辑</a>
...
<script type="text/javascript" charset="utf-8">
	function del(val) {
		...
		layer.open({
			...
			$.ajax({
		        type: 'GET',
		        url: '<?php echo U("AdminAdvertisement/del");?>?id='+val, 
		        dataType: 'text',
