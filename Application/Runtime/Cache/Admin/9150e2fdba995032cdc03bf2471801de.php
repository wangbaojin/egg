		        ...
		        <table class="table">
		            ...
		            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr style="border: 1px solid rgb(206,206,206);">
		                    <td  style="padding: 10px;width: 100px">
		                        ...
		                        <?php echo ($vo["created_date"]); ?><br>
		                        <?php echo ($vo["state_info"]); ?>
		                    </td>
		                    <td  style="padding: 10px">
		                    	<?php echo ($vo["user_info"]["full_name"]); ?><br>
		                    	<?php echo ($vo["user_info"]["mobile"]); ?>
		                    </td>
		                    <td  style="padding: 10px">
		                        <b><?php echo ($vo["apply_amount"]); ?> </b>元
		                    </td>
		                    <td  style="padding: 10px">
		                    	提现支付宝账户:<b><?php echo ($vo["zhifubao_account"]); ?></b>
		                    </td>
		                    <td  style="padding: 10px">
		                    	<?php echo ($vo["note_msg"]); ?>
		                        <button onclick="setNote(<?php echo ($vo["id"]); ?>)" class="btn btn-info btn-xs">备注</button>
		                    </td>
		                    <td  style="padding: 10px">
		                        <?php if($vo['state'] == 1): ?><button onclick="agree(<?php echo ($vo["id"]); ?>)" class="btn btn-success btn-xs">同意</button>
		                        <button onclick="disagree(<?php echo ($vo["id"]); ?>)" class="btn btn-danger btn-xs">拒绝</button>
								<?php elseif($vo['state'] == 2 and $vo['pay_state'] == 2 ): ?>
									<button onclick="confirmPay(<?php echo ($vo["id"]); ?>)" class="btn btn-success btn-xs">确认放款</button>
								<?php elseif($vo['pay_state'] == 3): ?>
									<button onclick="completePay(<?php echo ($vo["id"]); ?>)" class="btn btn-success btn-xs">放款已完成</button>
								<?php else: ?>
								<?php echo ($vo["state_info"]); endif; ?>
		                    </td>
...
<script type="text/javascript" charset="utf-8">
	...
	function agree(val) {
		...
		layer.open({
			...
			$.ajax({
		        type: 'GET',
		        url: '<?php echo U("AdminWithdrawals/agreeWithdrawals");?>?id='+val, 
		        dataType: 'text',
	...
	function disagree(val) {
		...
		layer.open({
			...
			$.ajax({
		        type: 'GET',
		        url: '<?php echo U("AdminWithdrawals/disagreeWithdrawals");?>?id='+val, 
		        dataType: 'text',
		        success:function(msg){
		            if(msg=='success') {
		                layer.msg('成功!',{
	...
	function confirmPay(val) {
		...
		layer.open({
			...
			$.ajax({
		        type: 'GET',
		        url: '<?php echo U("AdminWithdrawals/confirmPay");?>?id='+val, 
		        dataType: 'text',
		        success:function(msg){
		            if(msg=='success') {
		                layer.msg('成功!',{
	...
	function completePay(val) {
		...
		layer.open({
			...
			$.ajax({
		        type: 'GET',
		        url: '<?php echo U("AdminWithdrawals/completePay");?>?id='+val, 
		        dataType: 'text',
		        success:function(msg){
		            if(msg=='success') {
		                layer.msg('成功!',{
	...
	function setNote(val) {
		...
		layer.prompt(function(value, index, elem){
			...
			$.ajax({
		        type: 'GET',
		        url: '<?php echo U("AdminWithdrawals/setNote");?>?id='+val+'&note_msg='+value, 
		        dataType: 'text',
		        success:function(msg){
		            if(msg=='success') {
		                layer.msg('成功!',{
