...
<script type="text/javascript" charset="utf-8" async defer>
    $.cookie('chicken_type','');
 	$.ajax({
        type: 'get',
        url: "http://wechat.jiagehao.cn/EggcoinChicken/getChickenType",
        dataType : 'json',
        success:function(res){
			...
			for(var i=0; i<data.length; i++){
				$(".type").push("<li id="+data[i].id+">"+
				"<div>"+
                    "<i>"+"</i>"
                    +"<span>"+data[i].discount_price+"/只</span>"
                    +"<span>"+data[i].discount+"/只</span>"
			...
			$(".type").push("</ul>");
			// document.body.innerHTML = $(".type").join("");
        },
        error:function(){
          	alert("未知错误!请稍后重试!");
        }
